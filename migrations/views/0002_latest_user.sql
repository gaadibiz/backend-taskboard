...
from user_fact uf
    INNER JOIN (
        ...
        from user_dim ud
            INNER JOIN (
                select
                    max(user_dim_id) as user_dim_id,
                    user_uuid
                from user_dim
                GROUP BY
                    user_uuid
    ...
    left join (
        ...
        from
            ...
            INNER JOIN (
                select
                    max(user_profile_id) as user_profile_id,
                    user_uuid
                from
                    user_profile
                GROUP BY
    ...
    ) as up on uf.user_uuid = up.user_uuid ORDER BY insert_ts DESC;