...
DROP PROCEDURE IF EXISTS analytics_purchase_order;

CREATE PROCEDURE `analytics_purchase_order`(
    IN startDate_in DATE,
    IN endDate_in DATE
)
BEGIN

    DROP Table IF EXISTS analytics_inventory_db.temp_purchase_order_test;
    DROP Table IF EXISTS analytics_inventory_db.temp_final_test_purchase_order;

    -- Create a temp table to target the necessary columns
    CREATE TABLE analytics_inventory_db.temp_purchase_order_test AS 
    SELECT 
        purchase_order_id,
        ...
        DATE(purchase_order_date) AS date_v 
    FROM 
        latest_purchase_order
    WHERE 
        DATE(purchase_order_date) >= startDate_in
        AND DATE(purchase_order_date) <= endDate_in;

    CREATE TABLE analytics_inventory_db.temp_final_test_purchase_order as 
    SELECT 
        UUID() as analytics_purchase_order_uuid,
        ...
        FROM
         analytics_inventory_db.temp_purchase_order_test t_inv
        GROUP BY t_inv.date_v
        ORDER BY t_inv.date_v;

    DELETE
        FROM
        analytics_inventory_db.analytics_purchase_order
        WHERE
        start_date IN (SELECT start_date from analytics_inventory_db.temp_final_test_purchase_order);

    -- Inserting data into Analytics table
    INSERT INTO analytics_inventory_db.analytics_purchase_order (
        calendar_id,
        analytics_purchase_order_uuid,
        start_date,
        ...
        SELECT 
        cal.calendar_id,
        temp.analytics_purchase_order_uuid,
        temp.start_date,
        ...
        FROM analytics_inventory_db.temp_final_test_purchase_order temp 
